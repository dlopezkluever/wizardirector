...
### 2.1 – Tag categories constant (`src/lib/constants/statusTags.ts`)

| # | Test | Steps | Expected Result |
|---|------|--------|-----------------|
| 2.1.1 | Predefined tags exist | (Code/integration) Import `COMMON_STATUS_TAGS` and inspect length. | Array has 18 entries (5 condition, 5 damage, 4 temporal, 4 appearance). |
| 2.1.2 | Category colors | (Code) Call `getTagCategory('muddy')`, `getTagCategory('torn')`, `getTagCategory('unknown_tag')`. | Returns `'condition'`, `'damage'`, `'custom'` respectively. |
| 2.1.3 | Tag colors | (Code) Call `getTagColors('bloody')`, `getTagColors('custom_xyz')`. | First returns amber-like classes (condition); second returns secondary/custom classes. |
| 2.1.4 | Suggestions empty query | Call `getTagSuggestions('')`. | Returns up to 8 tag names (e.g. first 8 from COMMON_STATUS_TAGS). |
| 2.1.5 | Suggestions with query | Call `getTagSuggestions('mud')`. | Returns suggestions containing "mud" (e.g. "muddy"), up to 8. |
| 2.1.6 | Suggestions with projectTags | Call `getTagSuggestions('', ['muddy','bloody'])`. | "muddy" and "bloody" appear first in the result, then other common tags, up to 8 total. |
...
| 4.1.2 | Clear search | With search non-empty, click the X in the search field (or clear manually). | Search clears and full list returns. |
