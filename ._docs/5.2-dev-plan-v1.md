...
- **Inheritance logic**: Scene 1 inherits from Master Assets; Scene N inherits from prior scene(s) end states
...
**Purpose**: Automatically detect which assets are needed for a scene based on the locked shot list. **Note:** Inheritance context uses *most recent instance per asset* in the branch (non-linear), not only N−1. Also, only existing project assets may be suggested; `new_assets_required` is advisory only—never auto-create.
...
export class SceneAssetRelevanceService {
   ...
   *Non-linear inheritance context; project-asset-only output.
   */
...
Lint is clean. If you want, we can add a quick manual test flow (e.g. empty scene → Detect → confirm suggestions and bulk generate).