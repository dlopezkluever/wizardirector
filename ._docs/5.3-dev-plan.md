...
### Recommended Order for Dev

**Phase 1: Task 2 only** (foundation)  
- Tag categories, colors, autocomplete in `statusTags.ts` + StatusTagsEditor.  
- **Test:** Add/remove tags in editor → correct colors and suggestions.  
- Do this first; everything else uses `getTagColors` / `getTagSuggestions`.

**Phase 2: Task 3 only** (list display)  
- Chips + “will carry forward” in SceneAssetListPanel.  
- **Test:** List shows tags and carry-forward hint.  
- Small, depends only on Phase 1.

**Phase 3: Task 4 only** (filter & search)  
- Search + filter dropdown + active filter badges.  
- **Test:** Filter by tag / “has any tags” / name search; clear filters.  
- Depends on Phase 1 (colors for badges); nicer with Phase 2 (tags visible in list).

**Phase 4: Task 5 only** (carry-forward prompt)  
- TagCarryForwardPrompt modal + wiring in Stage 8.  
- **Test:** Two scenes, inherit assets with tags, enter Stage 8 for scene 2 → prompt → confirm/carry none.  
- Do alone; it’s the heaviest and needs a full flow check.

**Phase 5 (optional): Task 1, then Task 6**  
- Task 1: migration + trigger; quick test (add tags on draft scene → expect error).  
- Task 6: backend query params for filtering; test with `?tags=muddy` etc.  
- Both optional and independent of 2–5.

**Summary order:** **2 → 3 → 4 → 5**, then optionally **1 → 6**. Task 7 stays future. No need to group 2+3 or 4+5; each phase is one task with a clear test before moving on.
...
## Summary of what was implemented for **Task 5: Tag Carry-Forward Prompt (New Scene Initialization)**:

### 5.1 – TagCarryForwardPrompt component

**New file:** `src/components/pipeline/Stage8/TagCarryForwardPrompt.tsx`

- **TagCarryForwardDecision** – `instanceId`, `assetName`, `tagsToCarry`, `carryAll`
- **TagCarryForwardPromptProps** – `isOpen`, `onClose`, `priorSceneNumber`, `currentSceneNumber`, `priorInstances`, `onConfirm`
- Local state for per-asset and per-tag choices; initial state “carry all” for each instance
- **useEffect** to reset choices when the dialog opens with new `priorInstances`
- Handlers: **handleToggleAll**, **handleToggleTag**, **handleConfirm**, **handleCarryAll**, **handleCarryNone**
- UI: dialog with scrollable list of prior-scene assets, “Carry all” checkbox per asset, clickable tags (selected vs unselected) using **getTagColors** from `@/lib/constants/statusTags`
- Info note that tags apply to the starting state of the current scene and can be edited later
- Footer: “Carry None”, “Carry All”, “Confirm Selection”
- Summary line: “X of Y tags selected”

### 5.2 – Integration in Stage 8

**Updated:** `src/components/pipeline/Stage8VisualDefinition.tsx`

- Imports: **TagCarryForwardPrompt**, **TagCarryForwardDecision**
- **currentSceneNumber** state, set in the existing scene-fetch effect when the current scene is found by `sceneId`
- **showTagCarryForwardPrompt** and **priorInstancesWithTags** state
- **Effect:** when `sceneAssets` has inherited instances with at least one tag and a prior scene exists, and `localStorage` key `tag-prompt-${sceneId}` is not set, set `priorInstancesWithTags` (current-scene inherited instances + asset names) and open the prompt
- **handleTagCarryForwardConfirm:** for each decision, call `sceneAssetService.updateSceneAsset(projectId, sceneId, decision.instanceId, { statusTags: decision.tagsToCarry, modificationReason: … })`, then set `tag-prompt-${sceneId}` in `localStorage`, invalidate scene-assets query, and show success/error toasts
- **TagCarryForwardPrompt** rendered after **CostConfirmModal**, with **onClose** also setting `tag-prompt-${sceneId}` so dismissing the dialog counts as “already prompted” for that scene

Behavior matches the plan: first-time entry to Stage 8 for a scene with inherited instances that have tags triggers the prompt; choices are applied as updates to the **current** scene’s instances (hard copy of tags); “already prompted” is tracked per scene via `localStorage`. Downstream invalidation / “Apply to all subsequent scenes” remains out of scope (Phase 10).

## NOTE: Delaying Task 1, 6, & 7.