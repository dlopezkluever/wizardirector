...
export function getTagSuggestions(query: string): string[] {
  ...
  if (!q) return COMMON_STATUS_TAGS.slice(0, 8).map(t => t.name);
  
  return COMMON_STATUS_TAGS
...
- Autocomplete suggestions in StatusTagsEditor
...
2. **Efficient discovery** via autocomplete and filtering
3. **Continuity enforcement** via carry-forward prompts
4. **Database safety** via optional validation trigger
