...
### Branch Scope and Data Model (Already Enforced ✅)

**Branch scope:** Tag operations are already branch-scoped. All scene asset routes in `backend/src/routes/sceneAssets.ts` validate that the requested scene belongs to `project.active_branch_id` (e.g. `.eq('branch_id', project.active_branch_id)` on the `scenes` table). Therefore:
- Tags applied in a "What-if" branch never appear in Main (or any other branch) unless a merge occurs.
- No new "Task 0" or code change is required for branch isolation; it is inherent in the scene→branch relationship.

**Why no "Task 0: Branch-Awareness"?** The codebase already enforces branch scope (scene→branch in every route). A separate implementation task would add no new code; documenting the constraint here and deferring merge/conflict UI to Phase 10 keeps the plan accurate without scope creep.

**Deferred to Phase 10 (Version Control & Branching):** Explicit branch comparison UI, merge logic, and conflict resolution for tags are out of scope for 5.3. See implementation-task-list.md Features 10.1–10.5.

**Hard copy (no pointers):** Each scene has its own `scene_asset_instances` rows. When the user accepts a carry-forward prompt, we create or update **new rows** for the destination scene—we do not store pointers to prior-scene tags. Modifying tags in Scene 5 does not retroactively change Scene 4. This decoupling is already the data model.

**Downstream invalidation (Phase 10):** If a user edits tags in a "middle" scene (e.g. Scene 3 of 10), a "Propagate changes downstream?" prompt and an "Apply to all subsequent scenes" bulk action are **deferred to Phase 10** (Feature 10.5: Invalidation Logic). Implementing this in 5.3 would add significant modal fatigue and complexity; the plan keeps carry-forward as a **first-time scene entry** flow only.
...
### Integration with Stage 10 (Contract Only)

**Source of truth:** `status_tags` on scene asset instances are the **source of truth** for visual conditions at scene start. Stage 10 (Frame/Video generation) should consume them when building prompts.

**Contract (implementation in Stage 10):** A utility such as `getAssetVisualDescription(instanceId)` should concatenate the instance's `effective_description` with active `status_tags` (e.g. "John, with muddy clothes and torn fabric..."). Full implementation belongs to Stage 10 scope; 5.3 only ensures tags are stored and editable in Stage 8.
...
## REVISION CHECKLIST (Post-Recommendations)

- [x] **Branch scope:** Documented in "Branch Scope and Data Model"—tag operations are scoped to `active_branch_id` via scene→branch; no new Task 0; merge/conflict UI deferred to Phase 10.
- [x] **Hard copy:** Task 5 and Architecture explicitly state that carry-forward creates/updates new rows per scene; no pointers; edits in Scene N do not change Scene N−1.
- [x] **Downstream / bulk propagate:** Explicitly deferred to Phase 10 (Feature 10.5: Invalidation Logic); "Apply to all subsequent scenes" belongs there to avoid modal fatigue in 5.3.
- [x] **Stage 10 path:** "Integration with Stage 10" subsection and Task 7 note that `status_tags` are source of truth and should be consumed by Stage 10 (e.g. `getAssetVisualDescription(instanceId)`); implementation is Stage 10 scope.
- [x] **Autocomplete consistency:** Task 2 adds prioritization of tags already used in the project/branch to reduce "muddy" vs "dirt-covered" inconsistency.
