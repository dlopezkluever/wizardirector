...
# *Implementation Summary*:
  ...
  I've successfully implemented Stage 11 (Confirmation & Gatekeeping) as     
  specified in the plan. Here's what was created:

  Database Migrations

  1. backend/migrations/020_user_credits.sql - User credits table with       
  balance tracking, low credit threshold, and RLS policies
  2. backend/migrations/021_video_generation_jobs.sql - Video generation jobs   table with full job tracking (status, costs, frames, errors, provider     
  metadata)

  Backend Services

  3. backend/src/services/userCreditsService.ts - Manages user credit balance   with methods:
    - getBalance() - Returns balance with low-credit flag
    - ensureUserCredits() - Creates record if missing (100 credits default)  
    - deductCredits() / addCredits() - Balance modifications
  4. backend/src/services/videoGenerationService.ts - Video generation with  
  pricing constants:
    - VEO_31_FAST_RATE = 0.15 ($/second)
    - VEO_31_STANDARD_RATE = 0.40 ($/second)
    - getSceneCostBreakdown() - Full checkout data
    - confirmAndQueueRender() - Queue video jobs
    - getVideoJobs() - For Stage 12 display

  Backend Routes

  5. backend/src/routes/checkout.ts - API endpoints:
  ...
  6. backend/src/server.ts - Registered checkout router

  Frontend Types & Services

  7. src/types/scene.ts - Added checkout types: ShotCheckoutDetail,
  CheckoutData, DependencyWarnings, VideoGenerationJob, etc.
  8. src/lib/services/checkoutService.ts - Frontend API client

  Frontend Components

  9. src/components/pipeline/Stage11Confirmation.tsx - Full rewrite with:    
    - React Query data fetching
    - Model variant selector (Fast vs Standard)
    - Per-shot cost breakdown with expandable details
    - Credit balance display with low-credit warning
    - Dependency warnings (unapproved frames, prior scene issues)
    - Frame status badges on thumbnails
  10. src/components/pipeline/Stage12VideoGeneration.tsx - Queue status view 
  with:
    - React Query polling (3s interval while jobs active)
    - Job list with status indicators
    - Progress bar showing completion percentage
    - Video preview player for completed jobs
    - Issue resolution panel (only when complete)
  11. src/pages/ProjectView.tsx - Updated to pass projectId to Stage 11 and  
  Stage 12