...
## Aditional: Asset Naming When Merging/Matching Asset issue"  
## Summary of Changes

### 1. **Type Updates** (`src/types/asset.ts`)
   - Added `nameStrategy?: 'project' | 'global' | 'custom'` to `CloneAssetRequest`
   - Added `customName?: string` to `CloneAssetRequest`

### 2. **Modal UI** (`src/components/pipeline/AssetMatchModal.tsx`)
   - Added name selection section before the description strategy
   - Three radio options:
     - **Option 1 (default)**: Project asset name (e.g., "Project-Man")
     - **Option 2**: Global name with project name in parentheses (e.g., "Global-Man (Project-Man)")
     - **Option 3**: Custom name with editable text input and preview showing "(Project-Man)" appended
   - Added validation to ensure custom name is provided when option 3 is selected

### 3. **Service Layer** (`src/lib/services/projectAssetService.ts`)
   - Updated `cloneFromGlobal()` to accept `nameStrategy` and `customName` in options
   - Passes these parameters to the backend API

### 4. **Backend** (`backend/src/routes/projectAssets.ts`)
   - Added handling for `nameStrategy` and `customName` in the clone endpoint
   - Logic:
     - `'project'`: Uses extracted project asset name as-is
     - `'global'`: Uses `"${globalAsset.name} (${extractedAsset.name})"`
     - `'custom'`: Uses `"${customName.trim()} (${extractedAsset.name})"`
   - Defaults to project name if no strategy is provided

The name selection appears before the description strategy, defaults to the project asset name, and preserves script references. All changes pass linting with no errors.

-------------------------------------------------------------------------

## Addressing Tasks 7 & 8

### Task 7: Asset Drawer Integration — Already complete

What's implemented:
- AssetDrawer opens match modal when cloning (lines 144-146)
- Fetches extracted assets for current project (lines 138-139)
- Handles both matched and unmatched clone flows (lines 144-161)

Status: Done

---

### Task 8: Stage 5 Integration — Partially complete

What's implemented:
- Extracted assets are available for matching (fetched in AssetDrawer)
- Asset list refresh after matching (via `handleAssetCloned` callback in Stage5Assets.tsx line 309-313)

What's missing:
- Visual indicators for matched assets (no UI indication that an asset was matched with a global asset)

Status: 2/3 complete

---

## Pros and cons of completing Task 8 (visual indicators)

### Pros of adding visual indicators
1. User clarity: Users can see which assets came from global vs. extracted
2. Debugging: Easier to verify matching worked
3. Workflow transparency: Clear distinction between matched and unmatched assets
4. Future sync features: Foundation for showing sync status later

### Cons of adding visual indicators
1. UI complexity: Adds visual noise to Stage 5
2. MVP scope: Not critical for core functionality
3. Development time: Requires design and implementation
4. Potential confusion: May raise questions about what "matched" means

---

## Recommendation: Defer Task 8 visual indicators

### Why defer
1. Core functionality works: Matching, merging, and refresh are functional
2. MVP focus: Visual polish can wait
3. Low user impact: Users can verify matching via the modal; the list refresh confirms it
4. Future enhancement: Can be added when needed

### What to keep
- Task 7 is complete and working
- Task 8 refresh functionality (already working)
- Matching functionality (already working)

### What to defer
- Visual indicators for matched assets (Task 8, last bullet)
...
2. Mark Task 8 as mostly complete, with visual indicators deferred
3. Add a future enhancement note:
   ```
   Future Enhancement: Visual Indicators for Matched Assets
   - Add badge/icon to show which assets were matched with global assets
   - Could show sync status, version info, etc.
...
Defer the visual indicators. The matching workflow is functional, and adding indicators now would:
- Add complexity without critical value
- Delay MVP progress
- Be easy to add later when needed

The current implementation is sufficient for MVP. Users can:
- Match assets via the modal
- See the list refresh after matching
- Verify matched assets by checking their `global_asset_id` in the database (if needed for debugging)

Add visual indicators later when:
- Users request it
- You're adding sync/version features
- You have time for polish

Bottom line: Ship the MVP with current functionality, and add visual indicators as a future enhancement.