...
- ✅ **IN SCOPE:** Infrastructure (database, API, types) for visual frame preview
- ✅ **IN SCOPE:** Integration into **functional stages** (Stage 7, Stage 10)
- ❌ **OUT OF SCOPE:** Integration into Stage 8 & 9 (deferred to Phase 5-6)
...
**Current Status**:
- ✅ UI component fully implemented (`RearviewMirror.tsx`)
- ✅ Text-mode integration in Stage 7 (showing prior scene end state)
- ✅ Visual-mode placeholder in Stage 10
- ⏳ **Missing**: Real visual frame preview data flow (backend → database → frontend)
- ⏳ **Missing**: Real data in Stage 10 (currently using placeholder)
- 🚫 **Deferred**: Integration into Stage 8 & 9 (pending Phase 5-6)
...
### Task 0: *DONE* Prerequisite Verification (15 min) ⚠️ CRITICAL *DONE*
**Priority**: **HIGH** - Must complete before other tasks  
**Files**: `src/pages/ProjectView.tsx`, Stage 8/9 component files

**Discovery Summary:**
- ✅ Stage 7: Has `projectId` and `sceneId` props - fully functional
- ✅ Stage 10: Has `sceneId` prop - functional (needs real data)
- ❌ Stage 8: Missing `projectId` prop, using mock data (deferred to Phase 5)
- ❌ Stage 9: Missing `projectId` prop, using mock data (deferred to Phase 6)
...
{sceneStage === 8 && (
  <Stage8VisualDefinition 
    sceneId={activeSceneId}  // ✅ HAS sceneId
    // ❌ MISSING projectId
...
interface Stage8VisualDefinitionProps {
  sceneId: string;
  // ❌ projectId not in interface
  onComplete: () => void;
  onBack: () => void;
}
```

**Decision:** ✅ **Defer Stage 8/9 integration to Phase 5-6**  
...
export interface Scene {
  ...
  endFrameThumbnail?: string; // ✅ Already exists!
  shots: Shot[];
...
**Deliverable**: Type system supports end frame thumbnails ✅
...
**Behavior**:
- If prior scene has `endFrameThumbnail` → show visual mode
- If thumbnail fails to load → fallback to text mode
- Show loading spinner while thumbnail loads
- If no thumbnail → text mode (current behavior)
...
8. Test with invalid URL → should fallback to text mode
...
<RearviewMirror
  mode="visual"
  priorEndFrame="/placeholder.svg" // ❌ Mock data
  priorSceneName="Scene 0: Prologue" // ❌ Hardcoded
/>
```
  ...
  if (!endFrameUrl) {
    ...
    try {
      ...
      if (error) {
        ...
        console.log('✅ End frame thumbnail saved for scene');
      }
    } catch (err) {
...
#### Test 3: Stage 7 with No End Frame (Current State) ✅
...
#### Test 4: Stage 7 with End Frame (After Manual Population) ✅
...
#### Test 7: Edge Cases ✅
- [ ] First scene (Scene 1) → RearviewMirror completely hidden
- [ ] Prior scene with neither `priorSceneEndState` nor `endFrameThumbnail` → RearviewMirror hidden
- [ ] Prior scene with only text → RearviewMirror shows text mode
- [ ] Prior scene with only thumbnail → RearviewMirror shows visual mode
- [ ] Page refresh preserves scene context (URL navigation already implemented)
- [ ] Network error during scene fetch → graceful error handling
...
- ✅ Backend returns `endFrameThumbnail` field
- ✅ Scene service transforms field correctly
- ✅ Manual test: Set thumbnail URL in DB, verify API returns it
...
**Exit Criteria:**
- ✅ Stage 7 RearviewMirror dynamic mode (visual/text)
- ✅ Stage 7 error handling and loading states
- ✅ Stage 10 uses real prior scene data
...
### No Changes Needed
1. `src/types/scene.ts` - Already has `endFrameThumbnail` field ✅
...
### Dependencies
- ✅ Supabase Storage already configured (from Phase 3)
- ✅ `sceneService.fetchScenes()` already working (from Phase 4.1-4.2)
- ✅ RearviewMirror UI component complete (from Phase 4.4 partial)
- ✅ Stage 7 already fetches scene data with `projectId`
- ⚠️ Stage 10 may need `projectId` prop added (verify in Task 6)

### Risks & Mitigations

**Risk 1**: Thumbnails not yet generated (no Stage 10 completion logic)  
**Impact**: Medium (expected)  
**Mitigation**: ✅ Build infrastructure now, gracefully degrade to text mode. Thumbnail population deferred to Phase 7 (Stage 10 implementation).

**Risk 2**: Large thumbnail files slow down scene fetching  
**Impact**: Low  
**Mitigation**: ✅ Use Supabase CDN URLs (already optimized). Consider lazy loading thumbnails in future if needed (>20 scenes).

**Risk 3**: Stage 10 missing `projectId` prop  
**Impact**: Medium (blocks Task 6)  
**Mitigation**: ✅ Task 6 includes verification step. If missing, add to interface and ProjectView (5 min).

**Risk 4**: End frame thumbnail URL format mismatch with Supabase Storage  
**Impact**: Low  
**Mitigation**: ✅ Use Supabase public URL format: `{supabaseUrl}/storage/v1/object/public/{bucket}/{path}`. Test with manual upload first in Phase 2.

**Risk 5**: Image load failures break UI  
**Impact**: Medium  
**Mitigation**: ✅ Added comprehensive error handling in Task 5 & 6. Falls back to text mode. No broken images visible.
...
- ✅ Database stores `end_frame_thumbnail_url` (nullable column)
- ✅ Backend API passes thumbnails when available
- ✅ Stage 7 displays visual preview when thumbnail exists
- ✅ Stage 7 falls back to text when thumbnail unavailable
- ✅ Stage 7 handles image load errors gracefully
- ✅ Stage 7 shows loading states while thumbnails load
- ✅ Stage 10 uses real prior scene data (not placeholder)
- ✅ First scene (no prior) handled gracefully (component hidden)
- ✅ All manual tests pass
- ✅ No console errors or warnings

### Out of Scope (Future Work):
- ⏳ Automatic thumbnail generation (Task 8 - Phase 7)
- ⏳ Stage 8/9 integration (Phase 5-6)
- ⏳ Thumbnail versioning
- ⏳ Full frames table implementation
- ⏳ Compare view modal (enhance RearviewMirror)
- ⏳ Thumbnail optimization/resizing
- ⏳ Lazy loading for >20 scenes
...
**Answer**: ❌ **NO** - Stage 8/9 only have `sceneId`, missing `projectId`
...
**Decision**: ✅ **Defer Stage 8/9 integration to Phase 5-6** (when they're rebuilt with real functionality)

---

### Q2: Should RearviewMirror be above or below stage-specific headers?
**Answer**: ✅ **Above** (before stage content, after main header)

**Rationale:**
- Matches Stage 7 and 10 current placement
- Continuity context should be first thing users see
- Mental model: "What came before?" → "What am I working on?"
...
**Answer**: ✅ **Use same format as Stage 10 generated frames**
...
**No additional thumbnail generation required** ✅

---

### Q4: Should we implement "Compare" button functionality now?
**Answer**: ❌ **No - defer to future enhancement**
...
**Answer**: ❌ **No - defer to Task 8 (Phase 7 implementation)**
...
**Feature 4.4 = Infrastructure Only** ✅
...
- ❌ Updating scene records
- ❌ Stage 10 completion handler
...
3. Mark Feature 4.4 as **COMPLETE** ✅
...
**Key Changes in v2.0:**
- ✅ Verified Stage 8/9 are mock implementations
- ✅ Deferred Stage 8/9 integration to Phase 5-6
- ✅ Added comprehensive error handling
- ✅ Added loading states
- ✅ Added rollback script to migration
- ✅ Clarified Task 8 (thumbnail population)
- ✅ Reduced timeline from 2.5 to 2 hours
...
### Task 2: Backend API Enhancement — done
...
### Task 3: Frontend type verification — done
...
### Task 5: Stage 7 – Use real end frame

**Stage7ShotList.tsx**
- **State**: Added `endFrame?: string` to `priorSceneData` and `imageError` state.
- **Fetch**: When loading the prior scene, set `endFrame: priorScene.endFrameThumbnail` and reset `setImageError(false)`.
- **RearviewMirror**:  
  - `mode={priorSceneData?.endFrame && !imageError ? 'visual' : 'text'}`  
  - `priorEndFrame={priorSceneData?.endFrame}`  
  - `onImageError={() => setImageError(true)}`  
  - Text fallback still uses `endState` or last 3 lines of `scriptExcerpt`.

**RearviewMirror.tsx**
- **Props**: `onImageError?: () => void`.
- **State**: `imageLoading` for the thumbnail.
- **Effect**: Reset `imageLoading` when `priorEndFrame` changes.
- **Visual mode**: Loading spinner while the image loads; `onLoad` / `onError`; on error call `onImageError()` so parents can switch to text mode.
- **Alt text**: Uses `priorSceneName` when provided.

### Task 6: Stage 10 – Real prior scene data

**Stage10FrameGeneration.tsx**
- **Props**: `projectId: string` added to the interface.
- **State**: `priorSceneData` (endState, endFrame, sceneNumber) and `imageError`.
- **Fetch**: `useEffect` loads prior scene via `sceneService.fetchScenes(projectId)` and fills `priorSceneData` when `currentSceneIndex > 0`.
- **RearviewMirror**: Replaced placeholder with real data:  
  - `mode={priorSceneData?.endFrame && !imageError ? 'visual' : 'text'}`  
  - `priorSceneEndState`, `priorEndFrame`, `priorSceneName` from `priorSceneData`  
  - `onImageError={() => setImageError(true)}`

**ProjectView.tsx**
- Stage 10 is rendered only when `projectId` is set, and `projectId` is passed into `Stage10FrameGeneration`.

### Resulting behavior

- **Stage 7**  
  - With prior scene and `endFrameThumbnail`: visual mode, thumbnail + loading spinner, then image or fallback to text on error.  
  - Without thumbnail or on image error: text mode (end state or script excerpt).

- **Stage 10**  
  - Same logic: visual when prior scene has a thumbnail and it loads; otherwise text; invalid URL triggers `onImageError` and fallback to text.

- **RearviewMirror**  
  - Loading state and error handling are centralized; parents control mode via `imageError` and optional `onImageError`.

Lint passes on the touched files. You can verify by: opening Stage 7 or 10 for scene 2+ (with prior scene), confirming RearviewMirror shows text when there’s no thumbnail, and when `end_frame_thumbnail_url` is set for the prior scene, confirming visual mode, spinner, and fallback to text for an invalid URL.