...
**Future optimization (Phase 5+):** Background extraction job

- Extract dependencies when scenes are persisted (Stage 4→5 transition)
- Trigger extraction as async job using task queue (Bull/BullMQ or Supabase Functions)
...
- Implement LLM extraction + regex fallback + fuzzy asset matching
...
- Add parallel extraction loop with `CONCURRENCY_LIMIT = 5`
