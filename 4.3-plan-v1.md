...
**Summary: Task 3 – Shot Service (Frontend)**
...
1. **ShotService class** with:
   - **fetchShots(projectId, sceneId)** – GET `/api/projects/:id/scenes/:sceneId/shots`, returns `Shot[]`
   - **extractShots(projectId, sceneId)** – POST `/api/projects/:id/scenes/:sceneId/shots/extract`, returns `Shot[]`
   - **updateShot(projectId, sceneId, shotId, updates)** – PUT `/api/projects/:id/scenes/:sceneId/shots/:shotId` with `Partial<Shot>` mapped to backend snake_case
   - **splitShot(projectId, sceneId, shotId, userGuidance?)** – POST `.../shots/:shotId/split`, returns `Shot[]`
   - **deleteShot(projectId, sceneId, shotId)** – DELETE `.../shots/:shotId`
   - **reorderShots(projectId, sceneId, orderedShotIds)** – PUT `.../shots/reorder`, returns `Shot[]`

2. **Auth** – Same pattern as other services: `supabase.auth.getSession()`, throw if no token, send `Authorization: Bearer <token>` on every request.

3. **Response handling** – `normalizeShot()` turns API responses (camelCase or snake_case) into the frontend `Shot` type so GET, extract, split, and reorder all return `Shot[]`.

4. **Updates** – `toBackendShotUpdate()` maps `Partial<Shot>` (camelCase) to the backend whitelist (snake_case: `characters_foreground`, `continuity_flags`, etc.). No-op when `updates` is empty.

5. **Errors** – Parse JSON when possible and throw with `error.error` or a fallback message.

6. **Export** – `export const shotService = new ShotService()` for use in Stage 7 and elsewhere.
...
- ✅ Replaced mock data with real API calls via `shotService`
- ✅ Automatic shot fetching on component mount
- ✅ Auto-extraction of shots from scene if none exist (using LLM)
- ✅ Proper loading states during fetch/extraction
- ✅ Comprehensive error handling with user feedback

#### **2. Debounced Auto-Save (Section 4.2)**
- ✅ Custom `useDebounce` hook (800ms delay)
- ✅ Tracks pending updates in a Map for batch saving
- ✅ Optimistic UI updates for instant feedback
- ✅ Auto-save indicator shows "Saving..." during save operations
- ✅ Error notifications if save fails
- ✅ "Unsaved changes" indicator in footer

#### **3. LLM-Based Shot Splitting (Section 4.3)**
- ✅ Modal dialog for shot splitting with optional user guidance
- ✅ Shows current shot action for context
- ✅ Loading state during AI processing
- ✅ Replaces original shot with two new shots in-place
- ✅ Auto-selects first new shot after split
- ✅ Toast notifications for success/failure

#### **4. Rearview Mirror Integration (Section 4.4)**
- ✅ Fetches prior scene data from `sceneService`
- ✅ Displays prior scene end state or last 3 lines of script
- ✅ Shows prior scene number for context
- ✅ Gracefully handles first scene (no prior data)
- ✅ Collapsible UI component

#### **5. Enhanced UI/UX (Section 4.5)**
- ✅ **Loading States**: Spinner with descriptive messages during fetch/extraction
- ✅ **Error States**: Clear error display with retry button
- ✅ **Empty States**: Helpful message when no shots available
- ✅ **Animated Shot Cards**: Smooth transitions with Framer Motion
- ✅ **Shot Inspector**: Comprehensive form with all shot fields
- ✅ **Visual Feedback**: 
...
1. **Mount**: Component loads → Fetches shots from API
2. **Empty State**: No shots → Auto-extracts using LLM → Displays shots
3. **Existing Shots**: Shots found → Displays immediately
4. **Edit Shot**: User types → Optimistic update → Auto-saves after 800ms
5. **Split Shot**: Click Split → Modal opens → Optional guidance → AI splits → Updates list
6. **Delete Shot**: Click Delete → Confirms → Removes from database → Updates list
7. **Complete**: Click "Lock Shot List & Proceed" → Advances to next stage
