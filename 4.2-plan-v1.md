...
todos:
  - id: database-migration
    content: "[PRIORITY 1] Add expected_characters, expected_location, dependencies_extracted_at columns to scenes table"
    status: pending
  - id: dependency-extraction-service
    content: "[PRIORITY 2] Create SceneDependencyExtractionService with LLM-based character/location extraction and fuzzy matching against Stage 5 master assets"
    status: pending
  ...
  - id: enhance-backend-endpoint
    content: "[PRIORITY 3] Enrich GET /api/projects/:id/scenes endpoint with cache-first parallel extraction (max 5 concurrent), dependency extraction, and continuity analysis"
    status: pending
  ...
  - id: frontend-type-handling
    ...
    status: pending
  - id: error-handling
    content: "[PRIORITY 4] Add timeout and fallback logic for LLM extraction failures"
    status: pending
  - id: testing-validation
    content: "[PRIORITY 5] Test dependency extraction, continuity warnings, caching, performance (<10s first load, <500ms cached), and error scenarios end-to-end"
    status: pending
isProject: false
---
...
graph TD
    ...
    Extractor -->|LLM extraction| Props[expectedProps]
...
ADD COLUMN expected_props TEXT[],      -- Array of prop names
...
- **⚠️ NEW: Fuzzy matching against Stage 5 master assets** - extracted character names and prop names should be matched against existing `global_assets` (type='character' and type='prop') to ensure consistency with visual profiles
...
interface SceneDependencies {
  ...
  expectedProps: string[]; // matched against master_assets where possible
}
...
export class SceneDependencyExtractionService {
  ...
  // NEW: Fuzzy match extracted prop names against project assets
  private async matchPropsToAssets(
...
3. **Fuzzy matching logic** (using Levenshtein distance with threshold 0.85):
   - **Step 1:** Clean strings (trim, lowercase) for both extracted names and asset names
   - **Step 2:** Check for exact match (after cleaning) - short-circuit for performance
   - **Step 3:** If no exact match, calculate similarity score using Levenshtein distance (or library like `string-similarity`)
   - **Step 4:** If similarity > 0.85, auto-match to asset name; otherwise, flag as "New" (use extracted name)
   - **Example mappings:**
     - "ALICE" → "Alice Smith" (if similarity > 0.85)
     - "JONATHAN" → "JONATHON" (handles typos, similarity > 0.85)
     - "BOB" → "Bob" (exact match after cleaning)
4. Return matched asset names when possible, fallback to extracted names if no match

**Prop Matching Strategy (NEW):**

1. Extract raw prop names from script excerpt (e.g., "a gun", "the letter", "his watch")
2. Query `global_assets` table for assets of type 'prop' linked to this project
3. **Fuzzy matching logic** (same as characters, threshold 0.85):
   - Handles variations like "a gun" → "The Golden Gun" (if similarity > 0.85)
   - "the letter" → "Love Letter" (if similarity > 0.85)
   - Falls back to extracted name if no match found
4. Return matched asset names when possible, fallback to extracted names if no match

**Rationale:** Scriptwriting is notorious for minor typos and casual references (e.g., "JONATHAN" vs "JONATHON", "a gun" vs "The Golden Gun"). If the system is too rigid, users will constantly see "New Asset Detected" warnings for assets they've already defined in Stage 5. The 0.85 threshold balances accuracy with tolerance for common variations. Props benefit from the same fuzzy matching approach as characters, ensuring consistency with the "Global-to-Local" architecture pattern.
...
const { data: scenes, error: scenesError } = await supabase
  .from('scenes')
  .select('id, scene_number, slug, status, script_excerpt, end_state_summary, updated_at, expected_characters, expected_location, expected_props, dependencies_extracted_at') // NEW: fetch cache columns
...
// NEW: Parallel extraction with concurrency limit (configurable via environment variable)
// Default: 5 concurrent LLM calls (adjustable for different rate limits: OpenAI Tier 1 vs Tier 5, GPT-4o vs GPT-4o-mini)
const CONCURRENCY_LIMIT = Number(process.env.EXTRACTION_CONCURRENCY) || 5;
  ...
  await Promise.all(
    batch.map(async (sceneIndex) => {
      ...
      try {
        ...
        scene.expected_props = dependencies.expectedProps;
      } catch (error) {
        ...
        scene.expected_props = scene.expected_props || [];
      }
    })
  );
}
...
- [ ] Character names are fuzzy-matched against `global_assets` using Levenshtein distance (threshold 0.85)
- [ ] Fuzzy matching handles typos correctly (e.g., "JONATHAN" matches "JONATHON" if similarity > 0.85)
- [ ] Exact match short-circuit works (lowercase + trim comparison before fuzzy matching)
...
- [ ] Parallel extraction processes up to N scenes concurrently (configurable via `EXTRACTION_CONCURRENCY` env var, default: 5)
...
**✅ IMPROVED MVP approach:** Cache-first extraction with parallel processing

- **Implementation:** Check cache before extracting; process up to 5 scenes concurrently
- **Pros:** 
  - 80% reduction in page load time (40s → 8s for 20 scenes on first load)
  - Subsequent loads are instant (<500ms)
  - Avoids 504 Gateway Timeout on Fly.io
  - Graceful degradation on LLM failures
- **Cons:** 
  - More complex logic (cache invalidation, concurrency management)
  - First-load experience still slower than ideal

**Future optimization (Phase 13+):** Background extraction job

- Extract dependencies when scenes are persisted (Stage 4→5 transition)
- Trigger extraction as async job using task queue (BullMQ/Redis) with Progress WebSockets
- Store in `expected_characters` and `expected_location` columns immediately
- Scene fetch becomes instant even on first load
- Eliminates user-facing latency entirely
- **Note:** This is logged as a placeholder task in `implementation-task-list.md` under "Phase 13: Performance & Scale Optimization" to prevent it from being forgotten during the "Polish" phase
...
**Trigger:** When the user edits the master script in Stage 4, the `scenes.script_excerpt` is updated

**Automatic handling:**
1. Migration 003's `update_scenes_updated_at()` trigger automatically updates `scenes.updated_at`
2. Endpoint checks: `if (dependencies_extracted_at < updated_at)` → re-extract
3. No manual cache clearing needed - timestamp comparison handles invalidation

**Edge cases:**
- If `dependencies_extracted_at` is `NULL` → always extract (first time)
- If `updated_at` is `NULL` (shouldn't happen) → always extract (defensive)
- If LLM fails → keep stale cached values, log warning, don't block response
...
- ✅ **Task 9: End-to-End Testing** - Full workflow validation
  ...
  - **Testing Checklist:**
    ...
    - ✅ `expectedProps` array populated from script (with fuzzy matching against Stage 5 assets)
...
cd backend
npm run migrate
# Verify: Check Supabase dashboard → scenes table has 3 new columns
```
...
- Implement LLM extraction + regex fallback + fuzzy asset matching (Levenshtein distance, threshold 0.85)
- Install dependency: `npm install string-similarity` (or similar library for fuzzy matching)
- Test with sample scene: "INT. KITCHEN - DAY" + dialogue with character names
- Test fuzzy matching: "JONATHAN" should match "JONATHON" if similarity > 0.85

**2B: Build Risk Analyzer**
- Create `backend/src/services/continuityRiskAnalyzer.ts`
- Implement 4-rule logic (see Task 3 for rules)
- Test with mock scene data (no prior, prior incomplete, upstream changed, etc.)

### Step 3: API Integration (1-2 hours)
- Modify `backend/src/routes/projects.ts` (lines 427-505)
- Add cache-checking logic: `if (!dependencies_extracted_at || dependencies_extracted_at < updated_at)`
- Add parallel extraction loop with `CONCURRENCY_LIMIT = Number(process.env.EXTRACTION_CONCURRENCY) || 5`
- **Note:** Make concurrency limit configurable via `EXTRACTION_CONCURRENCY` environment variable (default: 5)
- Update database cache after extraction
- Test: Load project with 20 scenes, verify <10s first load, <500ms second load

### Step 4: Frontend (1 hour)
- Update `src/lib/services/sceneService.ts` - add `continuityRisk: scene.continuityRisk || 'safe'`
- Update `src/pages/ProjectView.tsx` - ensure navigation handler updates URL with `?sceneId=...`
- Verify `src/components/pipeline/Stage6ScriptHub.tsx` - check line 119 passes sceneId correctly
- Test: Click "Enter Scene Pipeline", verify URL, refresh, confirm scene context preserved

### Step 5: Testing & Robustness (1-2 hours)
- Add timeout wrapper to extraction service (10s limit)
- Test LLM timeout scenario (mock slow response)
- Test empty scenes, missing data, rate limiting
- Run full E2E test suite (9 scenarios from Task 9)

### Debugging Checklist
- [ ] Migration ran without errors
- [ ] LLM extraction returns valid JSON `{ characters: [], location: "" }`
- [ ] Cache updates after extraction (check Supabase `dependencies_extracted_at` column)
- [ ] Second page load doesn't call LLM (check console logs)
