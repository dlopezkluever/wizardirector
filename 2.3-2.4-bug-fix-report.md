...
**Looking at**: 
```typescript
stage1Content.projectParams?.writingStyleCapsuleId  // ❌ Wrong path
```

**Should be**: 
```typescript
stage1Content.processedInput?.projectParams?.writingStyleCapsuleId  // ✅ Correct path
```
...
[ContextManager] Loaded writing style capsule: {capsule_name}          ← Now appears!
[API] Injected writing style context from Context Manager (X chars)    ← Now appears!
...
### Step 1: Create new project with writing style capsule ✅
- Capsule ID saved in `stage_states.content.processedInput.projectParams.writingStyleCapsuleId`

### Step 2: Generate treatments (Stage 2) 🔧 NOW FIXED
- ✅ Network tab includes `projectId`, `branchId`
- ✅ Context Manager assembles global context (no more errors)
- ✅ `writing_style_context` is injected (non-empty)
...
1. ✅ Database queries now use correct Supabase client methods
2. ✅ Context Manager now reads `writingStyleCapsuleId` from correct path in Stage 1 data

**Result**: The Context Manager can now successfully load and inject writing style capsules during Stage 2-4 generation.

**Next Step**: Re-run the testing checklist starting from Step 2.
