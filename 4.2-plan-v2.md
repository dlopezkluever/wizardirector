...
todos:
  - id: database-migration
    content: "[PRIORITY 1] ✅ COMPLETED - Migration 011 already created with expected_characters, expected_location, expected_props, dependencies_extracted_at columns"
    status: completed
  - id: dependency-extraction-service
    content: "[PRIORITY 2] Create simplified SceneDependencyExtractionService (no fuzzy matching) for Stage 4 integration"
    status: pending
  - id: stage4-frontend-integration
    content: "[PRIORITY 2] Integrate dependency extraction into scriptService.extractScenes() method"
    status: pending
  - id: stage4-backend-persistence
    content: "[PRIORITY 2] Update PUT /api/projects/:id/scenes endpoint to persist dependencies from frontend"
    status: pending
  - id: stage5-refactor
  ...
  - id: testing-validation
    content: "[PRIORITY 5] Test full workflow: Stage 4 extraction with dependencies → Stage 5 aggregation → Stage 6 instant load (<500ms)"
    status: pending
isProject: false
---
