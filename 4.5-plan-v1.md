...
**Task 7 – Shot service (lock/unlock)**  
- **`src/lib/services/shotService.ts`**
  - **`lockShotList(projectId, sceneId, force?)`**  
    - POSTs to `/api/projects/:id/scenes/:sceneId/shots/lock` with `{ force }`.  
    - On non-OK response, throws an error with `status` and `data` (errors/warnings) so the UI can show validation or force-lock.
  - **`unlockShotList(projectId, sceneId, options?)`**  
    - POSTs to `.../shots/unlock` with `{ reason, confirm }` for downstream-confirmation flows.

**Task 5 – Validation and auto-save race fix**  
- **`src/components/pipeline/Stage7ShotList.tsx`**
  - **State:** `validationErrors`, `validationWarnings`, `showValidationModal`, `isLocking`, `isEditable`.
  - **`validateShotsLocally()`**  
    - Same rules as plan: non-empty shots, unique shot IDs, required action/setting/camera, duration 1–30s (warnings for &lt;4s or &gt;12s), total duration vs shot count.
  - **`handleLockShotList()`**  
    - Sets `isEditable = false`, flushes all pending updates (bypasses debounce), waits 150ms, runs local validation, then either opens the validation modal or calls `lockShotList(false)`.
  - **`lockShotList(force)`**  
    - Calls API; on 409 sets warnings and reopens modal; on 400 sets errors and toasts; on success toasts and calls `onComplete()`.
  - **`getFieldValidationState(shot, field)`**  
    - Used for action, setting, camera, duration to drive error/warning borders.
  - Inspector inputs (duration, action, setting, camera, dialogue, characters) use `disabled={!isEditable}` and validation borders where applicable. Duration input max set to 30.
  - Footer “Lock Shot List & Proceed” uses `handleLockShotList`, shows “Locking...” when `isLocking`, and is disabled when `!isEditable || isSaving || !shots.length || isLocking`.

**Task 6 – Validation warning modal**  
- **`src/components/pipeline/Stage7ShotList.tsx`**
  - Dialog keyed by `showValidationModal`: title “Shot List Validation”, description varies for errors vs warnings.
  - **Errors:** red section with `AlertCircle`, list of errors; each row clickable to select that shot and close the modal.
  - **Warnings:** yellow section with `AlertTriangle`, same click-to-jump behavior.
  - **Footer:** “Fix Issues” / “Go Back” to close; “Lock Anyway” only when there are warnings and no errors, calls `lockShotList(true)` and sets `isLocking(true)` (cleared in `lockShotList` on error).

### Debugging:

#### 1:

Checking how the backend resolves the scene for the lock endpoint and how the frontend obtains scene IDs.

Identifying cause: the lock endpoint looks up the scene by `sceneId` and `project.active_branch_id`. "Scene not found" means no row in `scenes` with that id and branch. Adding clearer error handling and updating the testing doc with causes and a short checklist.

Summary of what’s in place:

**1. Backend (`backend/src/routes/projects.ts`)**  
- **Explicit “no active branch” check:** If `project.active_branch_id` is null, the lock handler now returns **400** with `{ error: 'Project has no active branch', code: 'NO_ACTIVE_BRANCH' }` instead of 404.  
- **Clearer 404:** When the scene isn’t found, the response now includes:
  - `code: 'SCENE_NOT_FOUND'`
  - `hint: 'Ensure the scene belongs to this project\'s active branch and was saved from Script Hub (Stage 6).'`
- **Logging:** A warning is logged with `projectId`, `sceneId`, and `active_branch_id` whenever the scene lookup fails so you can see why in the server log.

**2. Testing doc (`4.5-testing-1.md`)**  
- Explains **why** you get 404: scene must exist and have `scenes.branch_id = project.active_branch_id`.  
- Lists **likely causes:** no active branch (now 400), scene from another project/branch or URL, or stale scene id.  
- Describes the **backend changes** above.  
- Gives **steps to reproduce** correctly: one project → Stage 6 → Enter a scene from the list → Stage 7 → Lock.  
- Suggests checking the **backend log** for `[shots/lock] Scene not found:` and optionally checking the **DB**: `projects.active_branch_id` and `scenes.branch_id` for that scene.

**Next steps for you**  
1. Reproduce using only “Enter” from the Script Hub list (no manual URL editing).  
2. If it still 404s, look at the backend terminal for the new warning; it will show whether `active_branch_id` is set and which scene id was requested.  
3. If you have DB access, run the suggested queries to confirm the scene’s `branch_id` matches the project’s `active_branch_id`.  

If the project was created without a branch (e.g. no seed that sets `active_branch_id`), you’ll now get **400 "Project has no active branch"** instead of 404, which points to fixing project/branch setup rather than scene lookup.

#### 2: 

Identifying the cause: scene lookup fails because the SELECT includes `metadata`, and the `scenes` table has no `metadata` column. Checking the lock and unlock handlers for `metadata` usage:


Summary of what was wrong and what was changed:

**Cause:** The lock handler was selecting and updating a `metadata` column on `scenes`. Your DB has no such column, so Supabase raised **`column scenes.metadata does not exist`**. That error was treated as “scene not found,” which produced the 404.

**Changes in `backend/src/routes/projects.ts`:**

1. **Scene SELECT** – Removed `metadata` from the selected columns so the scene query no longer references a missing column.
2. **Lock UPDATE** – Stopped writing `metadata` in the lock update. The forced-lock audit block is commented out with a note so it can be re-enabled once `scenes.metadata` exists.

Lock should work now. Try **Lock Shot List & Proceed** again; the scene should be found and the lock should succeed.

**Optional later:** To store forced-lock audit data (who forced, when, which warnings), add a `metadata` column to `scenes` (e.g. `JSONB`), then uncomment and re-enable the forced-lock metadata block in the lock handler.
...
1. **Auto-dismiss**  
   - Toasts now close on their own after **5 seconds** by default.  
   - Implemented by calling `dismiss()` in a `setTimeout` when `duration > 0`.

2. **`duration` option**  
   - You can pass `duration` in ms when calling `toast()`:  
     - `toast({ title: '...', duration: 3000 })` → closes after 3s.  
     - `toast({ title: '...', duration: 0 })` → does not auto-dismiss (stays until the user closes it).

3. **Cleanup delay**  
   - `TOAST_REMOVE_DELAY` was reduced from `1000000` (about 16 minutes) to **5000** (5 seconds), so after a toast is dismissed (by user or auto) it’s removed from state quickly.

All existing `toast({ title, description, variant })` calls now auto-dismiss after 5 seconds by default. For any toast that should stay until the user closes it, pass `duration: 0`.

---

## Summary of what was implemented for Tasks 8–11 (4.5 Shot List Validation & Locking):

### **Task 9: Type updates**
- **`src/types/scene.ts`**: Added `shotListLockedAt?: string` to `Scene`.
- **`src/lib/services/sceneService.ts`**: Included `shotListLockedAt` in the scene transform (from `shotListLockedAt` or `shot_list_locked_at`).

### **Task 10: Backend scene fetch**
- **`backend/src/routes/projects.ts`**: Added `shot_list_locked_at` to the GET `/api/projects/:id/scenes` SELECT and set `shotListLockedAt` on the transformed scene object returned to the frontend.

### **Task 8: Visual indicators**
- **`src/components/pipeline/Stage7ShotList.tsx`**:
  - **`isSceneLocked`**: Fetched on mount with prior-scene data from `sceneService.fetchScenes` (current scene’s `shotListLockedAt`).
  - **`canEdit`**: `!isSceneLocked && isEditable`; used for all inputs and actions.
  - **Disabled when locked**: All shot inputs (duration, action, dialogue, characters, setting, camera) use `disabled={!canEdit || isSaving}`. Split, Merge (and direction toggles), and Delete use `disabled={!canEdit || ...}`.
  - **Lock banner**: Blue banner above the inspector when locked: “Shot List Locked” + “Unlock” button.
  - **Unlock flow**: `handleUnlock()` calls `shotService.unlockShotList(..., { confirm: false })`. On 409 with `requiresConfirmation`, shows modal with frame/video counts and cost; “Unlock & Invalidate” calls unlock with `confirm: true` and then clears lock state.
  - **Footer**: Lock button shows “Shot List Locked” + lock icon when locked and is disabled when locked.
- **`src/lib/services/shotService.ts`**: Unlock errors now attach `status` and `data` (so the UI can handle 409 and show the confirmation modal).

### **Task 11: Testing & documentation**
- **Tests**: Existing `backend/src/tests/shotLocking.test.ts` covers `shotValidationService` (empty list, duplicates, required fields, duration, character warnings). All 15 tests pass. Integration tests for lock/unlock HTTP endpoints are described in `4.5-plan-v1.md` (Task 11); they require app + DB and are not added in this repo.
- **Docs**:
  - **`Phase-4-Status-Implementations.md`**: Feature 4.5 marked complete with checklist (validation, lock/unlock, visual indicators, types, backend fetch, tests, docs).
  - **`._docs/user-flow.md`**: Stage 7 gate and unlock flow described (validation rules, lock, unlock with confirmation when downstream work exists).
  - **`._docs/architecture-and-rules.md`**: “Stage 7 Shot List Validation Rules” added (errors vs warnings, lock/unlock behavior, `shot_list_locked_at`).

---

**Manual testing (from 4.5-plan-v1.md):** Run the 7 critical scenarios (auto-save race, empty list, duplicate IDs, character typo, unlock with frames, status transition, force-lock audit). Backend tests: `cd "C:\Users\Daniel Lopez\Desktop\Aiuteur\wizardirector\backend"` then `npm run test` (shot locking tests pass; the only failure is the existing `image-generation.test.ts` TS error).